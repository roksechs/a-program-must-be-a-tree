...
| View & Physics | label mode, colour by kind or call height, layer gap and planes (off by default, with a focus-distance fade), auto-rotate, fit, top view (perspective-free, straight down the height axis); orbit around the selected subject by dragging (pitch never crosses over vertical, so the view never ends up upside down), pan with shift-drag, zoom with the wheel, or fly like an aircraft with the keyboard, eased in and out rather than snapping to speed — W/S pitch, A/D roll, Q/E yaw, ↑/↓ forward/back, ←/→ strafe, pitch and yaw turning around whichever way you're currently rolled so a bank-then-pitch is a real coordinated turn, with no limit or auto-level on roll (looping and flying upside down are fair game; "Fit to view" resets it); recompute (reheat) when the layout got stuck, reset positions, repulsion, spring stiffness, rest length |
