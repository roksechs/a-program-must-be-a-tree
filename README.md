...
| View & Physics | label mode, colour by kind or call height, layer gap and planes (off by default, with a focus-distance fade), auto-rotate, fit, top view (perspective-free, straight down the height axis); drag to look around like an FPS camera (turns in place from wherever you already are, not an orbit around a fixed subject, and never ends up "upside down" the wrong way — looping the view over is just where the look ends up), pan with shift-drag, the wheel or the keyboard's ↑/↓ dolly forward/back and ←/→ strafe sideways (an actual move through the scene, not a zoom), or fly like an aircraft with the rest of the keyboard, eased in and out rather than snapping to speed — W/S pitch, A/D roll, Q/E yaw, pitch and yaw turning around whichever way you're currently rolled so a bank-then-pitch is a real coordinated turn, with no limit or auto-level on roll (looping and flying upside down are fair game; "Fit to view" resets it); recompute (reheat) when the layout got stuck, reset positions, repulsion, spring stiffness, rest length |
