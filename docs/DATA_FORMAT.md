...
| field      | type            | notes |
|------------|-----------------|-------|
| `id`       | string          | Unique within the document. Convention: `<file>::<qualified name>`; a member is `<parent id>.<name>`, so a member bound by assignment in another file carries its parent's file in its id while `file` says where it was written. A binding on an undeclared global (`d3.scale = {}`) is `<file>::<full path>`. A local declaration is `<parent id>/<name>` (`/` marks lexical nesting, `.` membership): a function-valued object literal property passed as a call argument (`f({ m: () => {} })`) is always emitted this way, since it has no path of its own (docs/THEORY.md §4.1, Definition 9a); a named local function or a local `const x = () => {}` only appears this way with the analyzer's `--nested` option. Where that convention would produce the same id twice — two object literals in one declaration each carrying an `onclick`, neither of which has a name to be told apart by — the repeats after the first are suffixed `#2`, `#3`, … in source order. Uniqueness is not cosmetic: the viewer keys nodes by id, so declarations sharing one would collapse into a single node and the rest could never receive an edge. |
...
| `x`, `y`   | number          | A precomputed 2D position for this declaration, in the viewer's own layout units. Optional, and only useful in pairs: the viewer opens on the stored layout when *every* declaration carries one, and falls back to its deterministic seed otherwise rather than mixing settled nodes with nodes at the origin. Nothing about the graph depends on them — they are a starting point, and "Recompute (reheat)" replaces them. They exist because settling a graph is slow in a way no tuning fixes (the two forces are node-against-node, so a tick costs the whole graph however few edges it has): a 2,600-declaration codebase is still visibly moving at 800 ticks and only stops near 2,400, which is affordable in a build step and not on the machine of whoever opens the page. `npm run build:data` fills them in for the published datasets. |
...
| field    | type   | notes |
|----------|--------|-------|
| `source` | string | Id of the declaration whose body contains the reference (the caller). For `write`, reversed: the id of the variable being written. |
| `target` | string | Id of the referenced declaration (the callee). For `write`, reversed: the id of the declaration whose body writes it. |
| `kind`   | string | `call` (application of a function or method, including `super()`), `create` (`new X()`: the constructor that lookup finds for `X`, or `X` itself when no ancestor declares one), `reference` (value used without being applied, e.g. passed as a callback, stored or returned), `write` (the source is the target of an assignment, compound assignment or `++`/`--` in the target's body — edge direction reversed, from the variable to its writer, since the variable's next value depends on the writer and not the other way; a compound assignment also emits the ordinary `reference` edge for the read half), `type` (type-only use, including a call through an interface member), `extends`, `implements` (class to interface, or member to the interface member it implements), `override` (member to the member it overrides). Optional, defaults to `call`. See `THEORY.md`. |
