...
const STRINGS = {
  en: {
    ...
    "metric.activeEdges.hint": "Edges of the kinds ticked under Diagnostics in the Edges section.",
    ...
    "metric.scope": "Computed on the edge kinds ticked under Diagnostics in the Edges section (default: calls and constructions, the control graph).",
    ...
    "legend.edges": "Edges",
    "edges.draw": "Draw",
    "edges.springs": "Springs",
    "edges.metrics": "Diagnostics",
    "edges.help": "Each edge kind can be drawn, act as a spring in the physics, and count for degrees, call heights and the diagnostics, independently. The defaults draw and pull on every kind but diagnose the control graph only.",
  ...
  ja: {
    ...
    "metric.activeEdges.hint": "エッジ セクションの「診断」で有効にした種別のエッジ数。",
    "metric.initCycles": "初期化サイクル",
    "metric.initCycles.hint": "定義時依存の循環に含まれる宣言。初期化前に読まれます。",
    "metric.scope": "エッジ セクションの「診断」で有効にした種別で計算しています（既定は呼び出しと生成、すなわち制御グラフ）。",
    ...
    "edges.springs": "ばね",
    "edges.metrics": "診断",
    "edges.help": "エッジ種別ごとに、描画するか、物理でばねとして働くか、次数・呼び出し高さ・診断に数えるかを個別に切り替えられます。既定では全種別を描画してばねにし、診断は制御グラフだけで行います。",
