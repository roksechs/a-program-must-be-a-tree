...
const STRINGS = {
  en: {
    ...
    "metric.elevationGaps.hint": "Calls grouped by how many layers of call height they skip: the 3D view's vertical axis, not the dominator tree. Gap 1 is a caller one layer higher than it needed to be; a large gap is a declaration near the top of the graph reaching straight down to one near the bottom, past everything in between.",
  ...
  ja: {
    ...
    "metric.elevationGaps.hint": "呼び出しを、高さ(3Dビューの縦軸そのもので、支配木ではありません)にして何層分を飛び越えているかで分類したものです。差1は本来1層高いだけで済んだはずの呼び出し元。差が大きいものは、グラフの上の方にある宣言が、間の層をすべて飛び越して下の方の宣言まで直接手を伸ばしている状態です。",
