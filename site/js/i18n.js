...
const STRINGS = {
  en: {
    ...
    "metric.entryPoints.hint": "Declarations that nothing else calls. A program has to start running somewhere — main, startup code, an event handler the browser or a framework invokes — and those places are exactly the ones nothing calls. So this list is every way in. A name here you did not expect is worth a look: nothing reaches it, which usually means either something outside the code calls it, or nobody does any more.",
    "metric.entryPoints.none": "Nothing here is uncalled: every declaration is reached from somewhere.",
    "metric.elevationGaps": "Elevation gaps",
    "metric.elevationGaps.hint": "Every declaration sits on a layer — what nothing calls at the top, what calls nothing at the bottom — and that is the height you see in the view. A call normally reaches the layer just below its caller. This counts, for each call, how many extra layers it jumps past: 0 is a normal step down, and the bigger the number, the further something high up reaches straight into a low-level detail without going through anything in between.",
    "metric.elevationGaps.summary": "{flat} calls reach only the layer right below them. Total gap: {gapSum}.",
    "metric.elevationGaps.range": "Highlight a range",
    "metric.elevationGaps.rangeCount": "{count} calls in range",
    "metric.elevationGaps.none": "Nothing skips a layer: every call reaches only the one right below it.",
    "metric.independence": "Independence",
    "metric.independence.hint": "For one declaration: how much of what it uses belongs to it alone, from 0 to 1. It is 1 when everything it calls could be moved inside it, because nothing else uses those. It falls as its dependencies turn out to be shared with other declarations, and falls further the further away those other users sit — sharing a helper with a neighbour costs less than sharing it with the far side of the program. The list is ordered by how much a declaration gives up in total, so one unavoidable shared dependency ranks below a dozen of them.",
    ...
    "metric.islands.hint": "A group of declarations that call each other, call nothing outside the group, and are called by nothing outside it either. Follow the arrows in any direction from one of them and you never arrive anywhere else in the program. On screen that is the clump you can see drifting away from the rest of the graph. It is usually one of two things: a family reached only from outside the code (an exported API, handlers a framework calls), or code nothing uses any more.",
  ...
  ja: {
    ...
    "metric.entryPoints.hint": "他のどこからも呼ばれていない宣言です。プログラムは必ずどこかから動き始めます(main、起動処理、ブラウザやフレームワークが呼ぶイベントハンドラなど)。そうした「最初に動く場所」は、まさに何からも呼ばれていない宣言です。つまりこの一覧が、このプログラムへの入り口すべてです。心当たりのない名前があれば要確認です。何からも到達されていないということは、コードの外から呼ばれているか、もう誰も使っていないかのどちらかです。",
    "metric.entryPoints.none": "呼ばれていない宣言はありません。すべてどこかから到達されています。",
    "metric.elevationGaps": "高低差",
    "metric.elevationGaps.hint": "すべての宣言は層に並びます。何からも呼ばれないものが一番上、何も呼ばないものが一番下で、これがビューで見えている高さそのものです。呼び出しは普通、すぐ1つ下の層に届きます。ここでは呼び出し1本ごとに、それが何層分を余計に飛び越えているかを数えています。0 が普通の1段下り。数が大きいほど、上のほうにあるものが間の層を一切通らずに、下のほうの細かい処理まで直接手を伸ばしている、ということです。",
    "metric.elevationGaps.summary": "{flat} 件の呼び出しは、すぐ1つ下の層にしか届いていません。高低差の合計: {gapSum}。",
    "metric.elevationGaps.range": "範囲を強調表示",
    "metric.elevationGaps.rangeCount": "範囲内の呼び出し: {count} 件",
    "metric.elevationGaps.none": "層を飛び越えている呼び出しはありません。どれもすぐ1つ下の層に届いています。",
    "metric.independence": "独立度",
    "metric.independence.hint": "1つの宣言について、使っているもののうちどれだけが自分専用かを 0〜1 で表します。1 は、使っているものすべてを自分の中に入れてしまえる状態(他の誰も使っていない)です。依存先が他の宣言と共有されているほど下がり、共有相手が遠いほど大きく下がります。すぐ隣と補助関数を分け合うより、プログラムの反対側と分け合うほうが高くつくからです。並び順は「合計でどれだけ手放しているか」なので、避けようのない共有が1件だけの宣言より、それが十数件ある宣言が上に来ます。",
    "metric.independence.of": "{score}(依存 {count} 件)",
    "metric.independence.none": "何にも依存していません。",
    "metric.islands": "浮島",
    "metric.islands.hint": "互いに呼び合っているだけで、組の外を呼ぶこともなく、外から呼ばれることもない宣言の組です。組の中のどれか1つから矢印をどちら向きに辿っても、プログラムの他の場所には決して行き着きません。画面上では、本体から離れて漂っている塊がそのまま該当します。多くの場合、コードの外側からしか呼ばれない一族(公開API、フレームワークが呼ぶハンドラ)か、もう誰も使っていないコードのどちらかです。",
